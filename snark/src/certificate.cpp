...
{
    libff::bit_vector answer = root;
    answer.insert(answer.end(), rk.begin(), rk.end());
...
std::string GenerateProof(std::string secStr, std::string randomKey){
    ...
    std::string secret = secStr;
    std::string random_key = randomKey;
    ...
    hash_bv = hash_two_to_one<HashT>(secNum_bv, rk_bv); //the leaf digest
    ...
    for (int i = 0; i < leaf_count-1; i++) {
        libff::bit_vector tmp = hash256<HashT>(salt[i]);
    ...
    */
    ...
    //std::cout << "leaf is " << *binToHex<HashT>(leaf) << std::endl;
    ...
    */
    ...
    /*  Below part only for debug purpose
...
bool VerifyProof (std::string randomKey, std::string rootStr){
    ...
    //load root
    /*  Below part load root in a txt file
